...
public class MaxentNERCrossValidator {
	String languageCode;
	...
	public MaxentNERCrossValidator(String languageCode, int cutoff, int iterations,
			AdaptiveFeatureGenerator featureGenerator) {
	...
	public void evaluate(ObjectStream<NameSample> samples, int nFolds) throws IOException {
		...
		while (partitioner.hasNext()) {
			CrossValidationPartitioner.TrainingSampleStream<NameSample> trainingSampleStream = partitioner
					.next();
			TokenNameFinderModel model = NameFinderME.train("en", "PHENOTYPE",
					trainingSampleStream, featureGenerator,
					Collections.<String, Object> emptyMap(), iterations, cutoff);
			TokenNameFinderEvaluator evaluator = new TokenNameFinderEvaluator(new NameFinderME(
					model), listeners);
